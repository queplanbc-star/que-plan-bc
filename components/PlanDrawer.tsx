...
export const PlanDrawer: React.FC<PlanDrawerProps> = ({
  ...
  const handleEditPlanName = (e: React.MouseEvent, plan: UserPlan) => {
    e.stopPropagation();
    setEditingPlanId(plan.id);
    setEditingPlanName(plan.name);
    window.history.pushState({ view: 'editing_plan_name' }, '');
  };

  const handleSavePlanName = async (e: React.MouseEvent, planId: string) => {
    e.stopPropagation();
    if (!currentUser || !editingPlanName.trim()) return;

    try {
      await updatePlanName(currentUser.uid, planId, editingPlanName.trim());
      onPlanCreated(); // Refresh plans
      if (selectedPlan?.id === planId) {
        setSelectedPlan({ ...selectedPlan, name: editingPlanName.trim() });
      }
      setEditingPlanId(null);
      window.history.back(); // Remove editing state from history
    } catch (error) {
      console.error(error);
    }
  };

  const handleCancelPlanNameEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingPlanId(null);
    window.history.back(); // Remove editing state from history
  };
            ...
            <div className="space-y-4">
              ...
              {selectedPlan.eventIds.length > 0 && (
                <button
                  ...
                  className="w-full md:w-auto bg-green-500 text-white font-bold py-3 px-6 rounded-full shadow-md hover:scale-105 transition-transform flex items-center justify-center gap-2 mb-4"
                >
                        ...
                        {editingPlanId === plan.id && plan.name !== 'Mis Favoritos' ? (
                          <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') handleSavePlanName(e as any, plan.id);
                                if (e.key === 'Escape') handleCancelPlanNameEdit(e as any);
                              }}
                              className="w-full p-1.5 text-sm border border-indigo-300 rounded focus:ring-2 focus:ring-indigo-500 outline-none"
                            />
                            <button
                              onClick={(e) => handleSavePlanName(e, plan.id)}
